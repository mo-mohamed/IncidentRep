defmodule IncidentReportWeb.Api.IncidentController do
  ...
  def create(conn, params) do
    ...
    paramaters_changset = param_validations(IncidentParamsValidation, [%IncidentParamsValidation{}, params]) |> IO.inspect(label: "validation")
    if paramaters_changset.valid? == false do
      ...
      params = Map.put(params, "local_image_path", file_name)
      ...
      case Incident.create(params) |> IO.inspect(label: "validation222") do
        {:ok, incident} ->
          conn
          |> put_status(:ok)
        {:error, changeset} ->   error_views(conn, 400, changeset: changeset)
      end
    end
  end
